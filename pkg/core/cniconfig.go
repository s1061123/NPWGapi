...
func GetCNIConfig(net *nptypes.NetworkAttachmentDefinition, confDir string) (config []byte, err error) {
	emptySpec := nptypes.NetworkAttachmentDefinitionSpec{}
	if net.Spec == emptySpec {
		...
		config, err = GetCNIConfigFromFile(net.Metadata.Name, confDir)
                if err != nil {
                ...
                // execute.
		config, err = GetCNIConfigFromSpec(net.Spec.Config, net.Metadata.Name)
                if err != nil {
